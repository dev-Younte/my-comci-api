...
/**
 * KST (한국 표준시) 기준의 YYYY-MM-DD 문자열을 안전하게 생성합니다.
 * 타임존 관련 오차를 완전히 배제하기 위해 사용됩니다.
 */
 ...
 */
function calculateWeekday(targetDateStr: string, weekStartDateStr: string): number {
  const [ty, tm, td] = targetDateStr.split('-').map(Number);
  const [sy, sm, sd] = weekStartDateStr.split('-').map(Number);
  
  // 연/월/일만 추출하여 자정(UTC) 기준으로 차이 계산 (시차/타임존 왜곡 방지)
...
export async function fetchComciTimetable(options: FetchTimetableOptions): Promise<TimetableResult> {
  ...
  const kstDateStr = getKstDateString(options.targetDate);
  ...
  // 5. 조회 대상일의 요일 인덱스 분석
  const weekdayIndex = calculateWeekday(kstDateStr, data.시작일);
  const dayNames = ['일요일', '월요일', '화요일', '수요일', '목요일', '금요일', '토요일'];
  
  // getDay()를 안전하게 구하기 (Date.UTC 활용)
