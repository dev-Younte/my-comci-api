...
export async function POST(request: Request) {
  try {
    const { name, studentId, phone, code, deviceId } = await request.json();
    ...
    const hashInput = studentId.trim() + phone.trim() + code.trim() + secret;
    const computedHash = createHash('sha256').update(hashInput).digest('hex');

    if (computedHash !== smsCode.code_hash) {
      return NextResponse.json(
    ...
    if (registeredDevice) {
      // 기존 기기가 있는 경우
      if (registeredDevice.device_id !== deviceId.trim()) {
