...
async function fetchLocationRules(classNum: number, forceRefresh: boolean = false): Promise<{ locations: Record<string, string>; bssids: Record<string, string> }> {
  const cacheKey = `loc_${classNum}`;
  const now = Date.now();
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [timetableResult, locationResult] = await Promise.all([
      ...
      fetchLocationRules(classNum, forceRefresh)
    ]);
