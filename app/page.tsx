        ...
        <section className="glass-panel" style={{
          ...
          <form onSubmit={handleFetch} style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
            ...
            >
              {loading ? (
                <>
                  <svg style={{ animate: 'spin 1s linear infinite', width: '20px', height: '20px' }} viewBox="0 0 24 24">
          ...
          <div style={{
            ...
            <div style={{
              ...
              {`/api/today-schedule?schoolCode=${schoolCode}&grade=${grade}&classNum=${classNum}${date ? `&date=${date}` : ''}${scriptUrl ? `&scriptUrl=${encodeURIComponent(scriptUrl)}` : ''}`}
            </div>
            <button
              onClick={() => handleCopy(`${window.location.origin}/api/today-schedule?schoolCode=${schoolCode}&grade=${grade}&classNum=${classNum}${date ? `&date=${date}` : ''}${scriptUrl ? `&scriptUrl=${encodeURIComponent(scriptUrl)}` : ''}`, 'api-url')}
              style={{
                fontSize: '0.75rem',
